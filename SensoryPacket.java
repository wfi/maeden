...
public class SensoryPacket {
	...
	/**
	 * Reads the data stored in a JSON Array into an array of Strings.  Initialize the status field from line 0
	 * <p>
	 * Entry0: smell (food direction)
	 * Entry1: inventory
	 * Entry2: visual contents
	 * Entry3: ground contents
	 * Entry4: messages
	 * Entry5: remaining energy
	 * Entry6: lastActionStatus
	 * Entry7: world time
	 *
	...
	protected String[] getRawSenseDataFromGrid(BufferedReader gridIn) {
		...
		try {
			...
			for (int i = 0; i < jsonArray.size(); i++) {
				result[i] = jsonArray.get(i).toString(); // fill the the result Array with the information.
			}
		} catch (Exception e){
			e.getMessage();
